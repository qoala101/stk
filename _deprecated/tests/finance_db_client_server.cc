...
void SetupDb(const cpp::Sp<stonks::StonksDb> &stonks_db) {
...
TEST(FinanceDbClientServer, Requests) {
  auto local_finance_db =
      cpp::MakeSp<stonks::finance::FinanceDb>(":memory:");
  auto server_finance_db =
      cpp::MakeSp<stonks::finance::FinanceDb>(":memory:");

  const auto server_db = stonks::server::StonksDb{6506, server_finance_db};
  auto finance_db_client = cpp::MakeSp<stonks::client::FinanceDb>(6506);
