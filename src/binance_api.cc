...
namespace {
std::optional<std::vector<stonks::binance::Symbol>> SymbolsFromExchangeInfo(
...
std::optional<std::vector<Symbol>> GetSymbols() {
  const auto exchange_info =
      rest::RestRequest{"https://testnet.binance.vision/api/v3/exchangeInfo"}
          .SendAndGetResponse();
...
std::optional<std::vector<Balance>> GetBalances() {
  auto request =
      rest::RestRequest{"https://testnet.binance.vision/api/v3/account"}
  ...
  const auto signed_params =
      utils::SignUsingHmacSha256(params, settings::SecretKey());

  const auto response =
      request.WithParameter("signature", signed_params).SendAndGetResponse();
