...
void v2_AddBodyToRequest(const v2_Body &request_body,
...
auto v2_SendRequest(const EndpointDesc &endpoint, RestRequest &request)
    -> web::json::value {
 ...
 * @throws If response misses mandatory body or body represents an exception.
 */
[[nodiscard]] auto v2_ParseResponseBody(
    const web::json::value &response_json,
...
class Client::Impl {
 public:
  ...
  [[nodiscard]] auto Execute(
    ...
    auto request = RestRequest{endpoint.method, base_uri_}.AppendUri(
        endpoint.relative_uri);
...
Client::Client(const Uri &uri)
    : impl_{std::make_unique<Impl>(uri)}, base_uri_{uri.GetFullUri()} {}
...
auto Client::v2_Execute(const EndpointDesc &endpoint, const v2_Params &params,
  ...
  v2_AddParamsToRequest(params, endpoint.v2_params, request);
  v2_AddBodyToRequest(request_body, endpoint.v2_request_body, request);
  auto response_json = v2_SendRequest(endpoint, request);
  return v2_ParseResponseBody(response_json, endpoint.v2_response_body);
}
}  // namespace stonks::network