...
namespace stonks::utils
{
  long UnixTimeMillis()
  {
    ...
    return long{std::chrono::duration_cast<std::chrono::milliseconds>(current_time).count()};
  }
  ...
  std::string SignUsingHmacSha256(const std::string &data, const std::string &key)
  {
    ...
    HMAC(EVP_sha256(), raw_key, key.length(), raw_data, data.length(), signed_data_array.data(), &signed_data_length);
