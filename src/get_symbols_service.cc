...
namespace
{
  ...
  {
    spdlog::info("Got {} request on {}", request.method(), request.request_uri().to_string());
    ...
    auto json_symbols = std::vector<web::json::value>(symbols.begin(), symbols.end());
...
namespace stonks
{
  ...
  {
    http_listener_ = web::http::experimental::listener::http_listener{"http://localhost:6506/api/symbols"};
    http_listener_.support(web::http::methods::GET, HandleGetRequest);
    return http_listener_.open();
  }

  pplx::task<void> GetSymbolsService::Stop()
  {
